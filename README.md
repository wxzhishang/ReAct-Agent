# ReAct Agent

基于 ReAct（Reasoning and Acting）架构的 AI Agent 实现，使用 TypeScript 和 Bun 运行时。

## 🌟 特性

- ✅ **ReAct 架构**：实现思考（Reasoning）和行动（Acting）的完整循环
- 🛠️ **可扩展工具系统**：轻松添加自定义工具
- 🌐 **真实 API 集成**：支持真实搜索和天气查询（可选）
- 💬 **交互式问答**：命令行交互式对话
- 🧠 **多轮对话上下文**：支持连续提问，自动记忆对话历史
- 🔄 **防循环机制**：智能检测和防止重复操作
- 📊 **详细日志**：可视化 Agent 的思考和决策过程
- 🔧 **内置工具**：计算器、搜索、天气查询
- 🚀 **高性能**：使用 Bun 运行时，快速高效
- 📝 **类型安全**：完整的 TypeScript 类型定义

## 📦 技术栈

- **Bun** - JavaScript/TypeScript 运行时
- **TypeScript** - 类型安全
- **OpenAI SDK** - LLM 调用（支持自定义端点）
- **Zod** - 数据验证
- **MathJS** - 数学计算
- **Consola** - 美观的日志输出

## 🚀 快速开始

### 1. 安装依赖

```bash
bun install
```

### 2. 配置环境变量

创建 `.env` 文件：

```bash
# OpenAI API 配置（必需）
OPENAI_API_KEY=your-api-key-here
OPENAI_BASE_URL=https://api.siliconflow.cn/v1
OPENAI_MODEL=Qwen/Qwen2.5-72B-Instruct

# 搜索 API 配置（可选 - 使用真实搜索）
# SERPER_API_KEY=your-serper-api-key

# 天气 API 配置（可选 - 使用真实天气）
# AMAP_API_KEY=your-amap-api-key
```

**获取 API Key：**
- SiliconFlow（推荐）: https://cloud.siliconflow.cn/account/ak
- Serper 搜索: https://serper.dev
- 高德地图开放平台: https://console.amap.com/dev/key/app

### 3. 运行程序

```bash
bun run start
```

### 4. 开始提问

```
🤔 请输入你的问题: 计算 123 + 456
🤔 请输入你的问题: 北京今天天气怎么样？
🤔 请输入你的问题: 搜索人工智能的最新进展
```

输入 `exit` 或 `quit` 退出程序。

## 📖 使用方法

### 交互式模式（推荐）

运行主程序，进入交互式问答：

```bash
bun run start
```

支持的命令：
- 直接输入问题 - Agent 会自动推理
- `exit` 或 `quit` - 退出程序
- `clear` - 清空屏幕
- `reset` - 清除对话历史
- `history` - 查看对话历史
- `Ctrl+C` - 强制退出

**多轮对话示例：**

```
🤔 请输入你的问题: 埃菲尔铁塔的高度是多少米？
✅ 答案: 埃菲尔铁塔的高度是324米。

🤔 请输入你的问题: 把刚才查到的高度乘以2再加上100，结果是多少？
✅ 答案: 计算结果是748米。(324 × 2 + 100 = 748)

🤔 请输入你的问题: 现在把原始高度和刚才的计算结果相加
✅ 答案: 324 + 748 = 1072米
```

Agent 会自动记住之前的对话内容，可以连续提问而无需重复说明上下文。

### 程序化使用

```typescript
import {
  ReActAgent,
  ToolRegistry,
  CalculatorTool,
  SearchTool,
  WeatherTool,
} from "./src/index.ts";

// 1. 创建工具注册表
const toolRegistry = new ToolRegistry();
toolRegistry.register(new CalculatorTool());
toolRegistry.register(new SearchTool());
toolRegistry.register(new WeatherTool());

// 2. 创建 Agent
const agent = new ReActAgent(
  process.env.OPENAI_API_KEY!,
  toolRegistry,
  {
    model: process.env.OPENAI_MODEL || "Qwen/Qwen2.5-72B-Instruct",
    baseURL: process.env.OPENAI_BASE_URL || "https://api.siliconflow.cn/v1",
    maxIterations: 10,
    temperature: 0,
    verbose: true,
  }
);

// 3. 运行查询（支持多轮对话）
const result1 = await agent.run("埃菲尔铁塔有多高？");
console.log(result1.answer); // 答案：324米

// 第二轮对话 - Agent 会记住第一轮的内容
const result2 = await agent.run("把刚才的高度乘以2");
console.log(result2.answer); // 答案：648米

// 管理对话历史
console.log(agent.getHistorySummary()); // 查看历史摘要
agent.clearHistory(); // 清除历史
const history = agent.getHistory(); // 获取完整历史
```

## 🛠️ 内置工具

### 1. Calculator（计算器）

执行数学计算，支持基本算术和数学函数。

```typescript
"计算 (123 + 456) * 789"
"sqrt(144) + 2^3 等于多少？"
```

### 2. Search（搜索）

在互联网上搜索信息。

- **真实搜索**：配置 `SERPER_API_KEY` 后使用 Google 搜索
- **内置知识库**：未配置时使用内置数据（fallback）

```typescript
"搜索人工智能的最新进展"
"埃菲尔铁塔是什么？"
```

### 3. Weather（天气查询）

查询城市天气信息（使用高德地图API）。

- **真实天气**：配置 `AMAP_API_KEY` 后使用高德地图实时天气数据
- **支持范围**：全国所有省市区县（3200+ 个城市）
- **查询模式**：支持实时天气和天气预报

```typescript
"北京今天天气怎么样？"
"查询杭州未来几天的天气预报"
"深圳现在的天气如何？"
```

## 🔧 自定义工具

创建自定义工具非常简单：

```typescript
import { z } from "zod";
import { Tool } from "./src/tools/base.ts";
import type { ToolResult } from "./src/agent/types.ts";

export class MyCustomTool extends Tool {
  name = "my_tool";
  description = "这是我的自定义工具";

  schema = z.object({
    input: z.string().describe("输入参数"),
  });

  async execute(input: any): Promise<ToolResult> {
    try {
      const parsed = this.schema.parse(input);
      
      // 实现你的工具逻辑
      const result = `处理结果: ${parsed.input}`;

      return {
        success: true,
        data: result,
      };
    } catch (error) {
      return {
        success: false,
        error: error.message,
      };
    }
  }
}

// 注册工具
toolRegistry.register(new MyCustomTool());
```

## 📂 项目结构

```
ReAct-Agent/
├── src/
│   ├── agent/
│   │   ├── react-agent.ts      # ReAct Agent 核心实现
│   │   └── types.ts             # 类型定义
│   ├── tools/
│   │   ├── base.ts              # Tool 基类和注册表
│   │   ├── calculator.ts        # 计算器工具
│   │   ├── search.ts            # 搜索工具（真实API）
│   │   ├── weather.ts           # 天气工具（真实API）
│   │   └── index.ts             # 工具导出
│   ├── prompts/
│   │   └── react-prompt.ts      # Prompt 模板
│   └── index.ts                 # 主入口
├── examples/                    # 示例代码
├── index.ts                     # 交互式主程序
├── package.json
├── tsconfig.json
└── README.md
```

## 🎯 ReAct 工作原理

ReAct (Reasoning and Acting) 是一种让 LLM 交替进行推理和行动的方法：

1. **Thought（思考）**：Agent 分析问题，思考需要做什么
2. **Action（行动）**：如果需要更多信息，选择并执行一个工具
3. **Observation（观察）**：观察工具返回的结果
4. **循环**：基于观察结果继续思考，直到能给出最终答案

### 示例执行流程

```
问题: "埃菲尔铁塔的高度乘以2是多少？"

迭代 1:
  💭 思考: 我需要先知道埃菲尔铁塔的高度
  🔧 行动: search
  📥 输入: {"query": "埃菲尔铁塔"}
  👀 观察: 埃菲尔铁塔高324米

迭代 2:
  💭 思考: 现在我知道高度是324米，需要计算324*2
  🔧 行动: calculator
  📥 输入: {"expression": "324 * 2"}
  👀 观察: 648

迭代 3:
  💭 思考: 我已经得到了答案
  ✅ 最终答案: 埃菲尔铁塔的高度是324米，乘以2等于648米
```

## 🧠 多轮对话上下文

Agent 支持多轮对话，能够记住之前的对话内容并在后续问题中引用。

### 工作原理

- 每次对话完成后，问题和答案会自动保存到对话历史中
- 下次提问时，Agent 可以访问完整的对话历史
- 支持引用之前的查询结果、计算结果等信息

### API 方法

```typescript
// 获取对话历史摘要
const summary = agent.getHistorySummary();
// 输出: "共 3 轮对话，6 条消息"

// 获取完整对话历史
const history = agent.getHistory();
// 返回: ConversationMessage[] 数组

// 清除对话历史
agent.clearHistory();
```

### 使用示例

查看 `examples/context-conversation-example.ts` 了解完整示例：

```bash
bun run examples/context-conversation-example.ts
```

**示例对话流程：**

```typescript
// 第一轮：查询信息
const result1 = await agent.run("埃菲尔铁塔的高度是多少米？");
// 答案：324米

// 第二轮：基于第一轮的结果计算（无需重复说明高度）
const result2 = await agent.run("把刚才的高度乘以2再加上100");
// 答案：748米 (324 × 2 + 100)

// 第三轮：引用前两轮的结果
const result3 = await agent.run("把第一轮的原始值和第二轮的结果相加");
// 答案：1072米 (324 + 748)
```

## ⚙️ 配置选项

```typescript
interface AgentConfig {
  model?: string;           // OpenAI 模型，默认 "Qwen/Qwen2.5-72B-Instruct"
  baseURL?: string;         // API 端点，默认 SiliconFlow
  maxIterations?: number;   // 最大迭代次数，默认 10
  temperature?: number;     // 温度参数，默认 0
  verbose?: boolean;        // 是否显示详细日志，默认 false
}
```

## 🌐 API 提供商

### SiliconFlow（国内推荐）

```bash
OPENAI_BASE_URL=https://api.siliconflow.cn/v1
OPENAI_MODEL=Qwen/Qwen2.5-72B-Instruct
```

### OpenAI 官方

```bash
# OPENAI_BASE_URL 留空或使用默认值
OPENAI_MODEL=gpt-4o-mini
```

### 其他兼容的 API

任何 OpenAI 兼容的 API 都可以使用。

## 💡 示例问题

### 计算类
- "计算 (123 + 456) * 789"
- "sqrt(16) + 2^3 是多少？"

### 搜索 + 计算组合
- "埃菲尔铁塔的高度乘以2是多少？"
- "长城的长度除以1000是多少？"

### 天气查询
- "北京今天天气怎么样？"
- "上海的温度乘以2是多少？"

### 复杂推理
- "北京和上海的温度相差多少度？"
- "搜索泰姬陵的信息，然后告诉我它建于哪个世纪"

## 🔍 调试

启用详细日志查看 Agent 的完整思考过程：

```typescript
const agent = new ReActAgent(apiKey, toolRegistry, {
  verbose: true,  // 启用详细日志
});
```

## 📜 脚本命令

```bash
# 运行交互式程序
bun run start

# 开发模式（自动重载）
bun run dev

# 类型检查
bun run type-check

# 运行示例
bun run example:basic
bun run example:custom
bun run example:siliconflow
```

## 📄 许可证

MIT

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📚 参考资料

- [ReAct Paper](https://arxiv.org/abs/2210.03629) - 原始论文
- [OpenAI API Documentation](https://platform.openai.com/docs)
- [Bun Documentation](https://bun.sh/docs)
- [SiliconFlow](https://cloud.siliconflow.cn/)
